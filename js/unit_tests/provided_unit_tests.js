...
describe("Insoo's unit test", function() {
  ...
  it('inserting data point', function() {
    var model = new ActivityStoreModel();
    var dataPoint = new ActivityData ('wc', {energy:3, stress:4, happiness:5}, 30);

    expect(model.getActivityDataPoints().length, 'model should have no data point').to.equal(0);
    model.addActivityDataPoint(dataPoint);
    expect(model.getActivityDataPoints().length, 'model should have 1 data point').to.equal(1);
    expect(model.getActivityDataPoints()[0], 'model should have the data point that was just inserted').to.equal(dataPoint);
  });

  it('removing data point', function() {
    var model = new ActivityStoreModel();
    var dataPoint = new ActivityData ('wc', {energy:3, stress:4, happiness:5}, 30);
  ...
  it('adding listener to model', function(){
    var model = new ActivityStoreModel();
    var dataPoint = new ActivityData ('wc', {energy:3, stress:4, happiness:5}, 30);
    var firstListener = sinon.spy();

    model.addListener(firstListener);

    model.addActivityDataPoint(dataPoint);

    expect(firstListener.called, "model listener should be called").to.be.ok;
    expect(firstListener.callCount, 'model listener should be called once').to.equal(1);
  });

  it('removing listener from model', function(){
    var model = new ActivityStoreModel();
    var dataPoint = new ActivityData ('wc', {energy:3, stress:4, happiness:5}, 30);
    var firstListener = sinon.spy();
    var secondListener = sinon.spy();
    ...
    model.removeListener(firstListener);

    model.addActivityDataPoint(dataPoint);

    expect(firstListener.called, "first listener should not be called").to.be.false;
    expect(secondListener.called, "second listener should be called").to.be.true;
  });

  it('Checking selectGraph for GraphModel', function() {
    var graphModel = new GraphModel();
    var firstListener = sinon.spy();
    ...
    expect(firstListener.called, 'GraphModel listener should be called').to.be.ok;
    expect(firstListener.calledWith('bar'), 'GraphModel argument verification').to.be.true;

    expect(graphModel.getNameOfCurrentlySelectedGraph(), 'selected graph name should be bar').to.equal('bar');
  });
