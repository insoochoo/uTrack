...
_.extend(ActivityStoreModel.prototype, {
    ...
    removeActivityDataPoint: function(activityDataPoint) {
        for(var i = 0; i < this.dataPoints.length; i ++){
            if(this.dataPoints[i] === activityDataPoint){
...
var GraphModel = function() {
    this.listeners = [];
    this.graphs = ['bar graph', 'table graph'];
    this.selectedGraph = 'bar graph';
};
...
_.extend(GraphModel.prototype, {
    ...
    removeListener: function(listener) {
        for(var i = 0; i < this.listeners.length; i++){
          if(this.listeners[i].toString() == listener.toString()){
...
function generateFakeData(activityModel, numDataPointsToGenerate) {
    var fakeActivities = ['wc','ed','ps','se','al','wt'];
